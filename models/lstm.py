...
class lstm(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, layers=1, nl=None, learn_init_hidden=False, criterion=None, nBatches=1):
        ...
        if learn_init_hidden:
            ...
            self.c0 = Parameter(torch.rand(nBatches, layers, self.nx))
        else:
            self.h0 = torch.zeros(nBatches, layers, self.nx)
            self.c0 = torch.zeros(nBatches, layers, self.nx)
